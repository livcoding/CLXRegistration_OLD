...
<body>
    <input type="text" id="_urls" hidden="true" value="<%=client_id%>,<%=redirect_uri%>,<%=oauth_server_url%>"/>
    ...
    <script>
        ...
        var doTokenTask = function (exp, $) {
             ...
             * childbrowser when the nvg context is not receiving the response,
             * instead the response is received on a child browser.
             */
            exp.nvg_checkfortoken = function (providerID, url, callback) {
                var
                        atoken,
                ...
                log("nvg_checkfortoken(" + providerID + ")");
                ...
                if (atoken.state && atoken.access_token) {
                    localStorage.setItem("auth_access_token", atoken.access_token);
                }
                if (atoken.state) {
             ...
             */
            var nvg_authrequest = function (providerid, scopes, callback) {
            ...
            debugger;
            exp.nvg_ensureTokens = function (ensure) {
                ...
                for (providerid in ensure) {
                    ...
                    if (token === null) {
                        nvg_authrequest(providerid, scopes);
                    }
                }
            ...
            exp.nvg_findDefaultEntry = function (c) {
                var
                        k,
                        i = 0;
            ...
            debugger;
            exp.nvg_configure = function (c, opts) {
                config = c;
                setOptions(opts);
                try {

                    var def = nvg_findDefaultEntry(c);
                    log("nvg_configure() about to check for token for this entry", def);
                    exp.nvg_checkfortoken(def);
            ...
            exp.nvg_getToken = function (providerid, scopes) {
            ...
            exp.nvg_registerRedirectHandler = function (callback) {
                api_redirect = callback;
            };

            exp.nvg_registerStorageHandler = function (object) {
                api_storage = object;
            };
            ...
            $.oajax = function (settings) {
                ...
                providerid = settings.nvg_provider;
                allowia = settings.nvg_allowia || false;
                scopes = settings.nvg_scopes;
                token = api_storage.getToken(providerid, scopes);
                co = config[providerid];
                ...
                if (!token) {
                    if (allowia) {
                        ...
                        nvg_authrequest(providerid, scopes, function () {
                            token = api_storage.getToken(providerid, scopes);
                            performAjax();
                        });
                        return;
        ...
        function getToken() {
            debugger;
            var url = document.getElementById("_urls").value.split(",");
            doTokenTask(window, window.jQuery);
            nvg_configure({
                "rest": {
                    client_id: url[0].trim(),
                    redirect_uri: url[1].trim(),
                    debug: true,
                    authorization: url[2].trim() + "/oauth/authorize"
                }
            });
            nvg_ensureTokens({
                "rest": ["read"]
