<%-- 
    Document   : summerRegistrationEntryList
    Created on : Jan 7, 2020, 5:24:38 PM
...
<form method="POST" id="ajaxform2" class="form-horizontal">
    ...
    <div class="row col-lg-7 form-group">
        <label style="text-transform: capitalize;text-align:right" class="col-sm-3 col-lg-5 col-xs-12 col-md-3 control-labe">Institute:<span style="font-size:19px;font-weight:bold;" class="text-danger"> *</span></label>
        <div class="col-sm-9 col-lg-6 col-xs-12 col-md-6">
            <select class="form-control" id="listmultiinstituteid" name="listmultiinstituteid" data-validation="required"data-live-search="true" onchange="getSemesterCode();">               
    ...
    <div class="row col-lg-7 form-group" >
        <label style="text-transform: capitalize;" class="col-sm-3 col-lg-5 col-xs-12 col-md-3 control-label">Enrollment No.:
...
<script>
    ...
    $("#listmultiinstituteid").chosen({width: "100%"});
    ...
    var cb = {
        add: {
            link: "summerRegistrationEntry/add",
            name: "Add",
            value: ""
        },
        edit: {
            link: "summerRegistrationEntry/edit",
            name: "",
            value: ""
        },
        delete: {
            link: "summerRegistrationEntry/delete",
            name: "Drop",
            value: ""
        },
        list: {
            link: "summerRegistrationEntry/list"
        },
        expression: {
            link: "",
            name: "",
    ...
    function getSemesterCode() {
        ...
        var instids = $('#listmultiinstituteid').val();
        $.ajax({
            type: "POST",
    ...
    function getStudentInfo() {
        ...
        if (listenrollmentNo != '') {
            $.ajax({
                type: "POST",
                url: "summerRegistrationEntry/getStudetnInfo",
                data: '&enrollmentno=' + listenrollmentNo + '&instituteids=' + instids,
                dataType: "json",
                async: false,
                timeout: 5000,
                cache: false,
    ...
    function getGridData() {
        ...
        if (studentid != '') {
            if (semestercode != '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "summerRegistrationEntry/getGridData",
                    data: '&registrationid=' + $("#semestercode").val() + '&studentid=' + $("#studentid").val() + '&instituteids=' + instids,
                    dataType: "json",
                    async: false,
                    timeout: 5000,
                    cache: false,
