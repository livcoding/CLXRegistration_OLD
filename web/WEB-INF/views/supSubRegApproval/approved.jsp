    ...
    <div class="col-lg-12" style="margin-top:10px;">        
        ...
        <div class="col-lg-12 form-group">              
            <div style="margin-top: 10px;margin-right: 15px;" class="form-group pull-right">
                <a href="javascript:cancelSelectedRecord();" class="btn btn-success btn-sm btn-flat">Cancel Selected Record</a>
            </div>
        </div>
    </div> 
...
<script>
    ...
    function loadApprovedData() {
        ...
        if (regCode != '') {
            ...
            $.ajax({
                url: "supplementarySubjectRagistrationApproval/getApprovedData",
                dataType: 'json',
                async: false,
                cache: false,
                contentType: false,
                ...
                success: function (data) {
                    ...
                    if (data.approvedData != null) {
                        ...
                        $.each(data.approvedData, function (i, item)
                        {
                            ...
                            var cou = 0;
                            str1 += ' <tr id="' + item[1] + '">';
                            str1 += ' <td id="' + item[1] + '"><input type="checkbox" id="approvedchk' + i + '" name="approvedchk' + i + '" value="' + item[1] + '~@~' + item[4] + '~@~' + item[7] + '~@~' + item[8] + '~@~' + item[9] + '~@~' + item[10] + '~@~' + item[11] + '~@~' + item[12] + '~@~' + item[13] + '"/>';
                            str1 += ' </td>';
                            str1 += ' <td>' + countt + '</td>';
                            str1 += ' <td>' + item[0] + '</td>';
                            str1 += ' <td>' + item[2] + '</td>';
    ...
    function cancelSelectedRecord() {
        ...
        $("#checkedCount2").val(checkedStatus);
        if (checkedStatus == 0) {
            BootstrapDialog.alert("Please select at least one check box..");
        } else {
            setFormIdAndUrl('ajaxform2', 'supplementarySubjectRagistrationApproval/cancel', 2);
        }
    }
